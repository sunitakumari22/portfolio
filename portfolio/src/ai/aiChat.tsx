...
export default function AiChat({ isOpen, setIsOpen }: AiChatProps) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch("https://portfolio-yjwu.vercel.app/api/chat", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ message: trimmed }),  // ✅ matches req.body.message in your controller
      });
      ...
      const assistantReply =
        data.reply ||                          // ✅ matches res.json({ reply: response }) in your controller
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "Oops! Something went wrong. Please check the server or try again later.",
        },
      ]);
    } finally {
      setIsLoading(false);
          ...
          >
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className={`text-sm font-semibold leading-tight ${theme === "dark" ? "text-white" : "text-gray-900"}`}>
                    AI Assistant
                  </p>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`p-1.5 rounded-lg text-xs transition-colors
                    ${theme === "dark" ? "text-gray-400 hover:text-gray-200 hover:bg-gray-800" : "text-gray-400 hover:text-gray-600 hover:bg-gray-100"}`}
                >
                  Clear
                </button>
                <button
            ...
            {/* Messages */}
            <div className={`flex-1 overflow-y-auto px-4 py-4 space-y-4 scroll-smooth ${theme === "dark" ? "bg-gray-950" : "bg-gray-50"}`}>
              {messages.map((msg, i) => (
                <div
                  key={i}
                ...
                >
                  <div
                    ...
                    ${msg.role === "assistant" ? "bg-gradient-to-br from-indigo-500 to-violet-600" : theme === "dark" ? "bg-gray-700" : "bg-gray-200"}`}
                  >
                    {msg.role === "assistant" ? (
              ...
              {isLoading && (
                <div className="flex items-start gap-2.5">
                  ...
                  <div className={`px-4 py-3 rounded-2xl rounded-tl-sm shadow-sm border ${theme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"}`}>
            ...
            {/* Input */}
            <div className={`px-3 py-3 border-t shrink-0 ${theme === "dark" ? "border-gray-700 bg-gray-900" : "border-gray-100 bg-white"}`}>
              <div className={`flex items-center gap-2 rounded-xl border px-3 py-2 transition-colors
                ${theme === "dark" ? "bg-gray-800 border-gray-700 focus-within:border-indigo-500" : "bg-gray-50 border-gray-200 focus-within:border-indigo-400"}`}
              >
                <input
                  ref={inputRef}
                  type="text"
                  ...
                  className={`flex-1 bg-transparent text-sm outline-none placeholder:text-gray-400 ${theme === "dark" ? "text-white" : "text-gray-900"}`}
                  disabled={isLoading}
                />
                <button
                  onClick={sendMessage}
